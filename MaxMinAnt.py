...
def acs():
    ...
    # Update delta tBest to 1 since we always consider best solution
    tBest = 1
    ...
    while(not(isBoardSolved(sudokuBoard))):
        ...
        # Do best value evaporation over time
        pBVE = 0.005
        tBest = (tBest * (1 - pBVE))
        ...
        for i in globalMatrix:
            ...
            for j in globalMatrix[i]:
                # Set Tmax = 1 pheromone limit
                if(((((1 - p) * j) +  tBest)) > 1):
                    newPheromones.append(1)
                else:
                    newPheromones.append(((1 - p) * j) +  tBest)
