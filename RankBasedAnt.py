...
def acs():
    ...
    while(not(isBoardSolved(sudokuBoard))):
        ...
        # Create ranking of ants
        rankedAnts = [] 
        rankedAnts.append(antArr[0])

        for i in range(len(antArr)):
            # Updated ranked ants list
            totalCells = antArr[i].numCellsSpecified - antArr[i].failCells
            if(i > 0):
                for j in range(len(rankedAnts)):
                    currentCells = rankedAnts[j].numCellsSpecified - rankedAnts[j].failCells
                    if(totalCells >= currentCells):
                        rankedAnts.insert(j + 1, antArr[i])
                    break
        ...
        # Rank based ant pheromone update

        # Loop through all solutions and apply pheromone based on ranking
        for a in range(len(rankedAnts)):

            # Calculate amount of pheromone to add, c is the total number of cells on the board and is weighted based on ranking of ants
            deltaT = (c / (c - ((rankedAnts[a].numCellsSpecified - rankedAnts[a].failCells) * (a / 10))))
